...
export default function AttendanceForm({mode,employeeId,stage='search'}:{mode:'checkin'|'checkout';employeeId?:string;stage?:'search'|'details'|'capture'}) {
  ...
  const [result,setResult]=useState<{message:string;timestamp?:string;email?:{status:string}}|null>(null);
  ...
  if(result)return <section className="card attendanceFlow"><span className="statusPill ready">Attendance saved</span><h1 style={{marginTop:24}}>{label} complete</h1><p>{employee?.full_name} · {employee?.employee_id}</p><p>{result.message}</p>{result.timestamp&&<p className="statusPill ready">{istDateTime(result.timestamp)} IST</p>}{photo&&<img className="attendanceReceiptPhoto" src={photo} alt="Photo saved with your attendance"/>}<p className="muted">{result.email?.status==='sent'?'Email notification sent.':'Attendance is saved. Email was not sent; the administrator can check notification logs and SMTP settings.'}</p><Link className="btn primary" href="/">Back to home</Link></section>;
  return <section className="card attendanceFlow"><ol className="flowSteps" aria-label="Attendance progress">{['Find profile','Confirm details','Photo & save'].map((text,index)=><li key={text} aria-current={index===(stage==='search'?0:stage==='details'?1:2)?'step':undefined}><span>{index+1}</span>{text}</li>)}</ol><p className="muted">{label} · Step {stage==='search'?1:stage==='details'?2:3} of 3</p><h1>{stage==='search'?'Find your profile':stage==='details'?'Confirm your details':'Take a live photo'}</h1>
