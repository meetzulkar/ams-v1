...
export default function HomeIdentity(){
 const [company,setCompany]=useState({companyName:'Attendance Pro',logoUrl:null as string|null});
 ...
 return <div className="homeIdentity"><div className="homeLogoFrame">{company.logoUrl&&!failed?<img className="homeCompanyLogo" src={company.logoUrl} alt={`${company.companyName} logo`} onError={()=>setFailed(true)}/>:<Building2 className="homeLogoFallback" aria-hidden="true"/>}</div><h1 className="homeCompanyName">{company.companyName}</h1><div className="homeLiveClock"><span className="eyebrow"><span className="onlineDot"/> LIVE · INDIA STANDARD TIME</span><time dateTime={now?.toISOString()} className="homeClockTime">{now?clockFormat.format(now):'--:--:--'}</time><p className="homeClockDate">{now?dateFormat.format(now):'Loading today’s date…'}</p></div></div>;
}
